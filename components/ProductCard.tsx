...
const WARNING_TEXT = "IMPORTANT: No text overlays, no captions, no visual elements on screen. Only the presenter and the product.";
...
export const ProductCard: React.FC<ProductCardProps> = ({ image, prompt, isGenerating, onRemove, isVeo3 = false }) => {
    ...
    const handleCopy = async (e: React.MouseEvent) => {
        ...
        try {
            ...
            // Ensure warning is present in the copied text
            if (!finalPrompt.includes("IMPORTANT: No text overlays")) {
                finalPrompt = `${finalPrompt.trim()}\n\n${WARNING_TEXT}`;
            }

            console.log("Attempting to copy full prompt:", finalPrompt);
    ...
    const handleCopyPart = async (index: number, part: ScriptPart, e: React.MouseEvent) => {
        e.stopPropagation();
        // Include the warning when copying individual parts to ensure compliance
